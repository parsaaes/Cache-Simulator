...
public class AddressGenerator {
    ...
    public void generate(String fileName){
        ...
        for (int i = startOfLocality; i< addresses.length; ) {
            String number = addresses[Math.abs(random.nextInt()) % (startOfLocality / 5 * (int)Math.ceil(( 3d * i / COUNT)))];
            addresses[i++] = number;
            if ((Math.abs(random.nextInt()) % 5) == 0) {
                long lengthOfLocality = Math.min(Math.abs(random.nextInt()) % 5,(MAX-toLong(number)));
